...
class WinAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        assert h * w == n, "Input token count must be a perfect square"

        x = x.permute(0, 2, 1).contiguous().view(b, c, h, w)

        # Pad if h or w is not divisible by window size
        ...
        x = x.view(b, h // self.window_size, w // self.window_size, self.window_size * self.window_size, c).cuda()
...
class CSAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        x_win = self.win_atten(x)  # (b, p, p, win, c)
        b, p_h, p_w, win, c = x_win.shape
        s = int(np.sqrt(win))
        h = x_win.view(b, p_h, s, p_w, s, c).permute(0, 1, 3, 2, 4, 5).contiguous()
        h = h.view(b, p_h * s, p_w * s, c).permute(0, 3, 1, 2).contiguous()  # (b, c, h, w)

        x_glob = self.dlightconv(x_win)
        atten_x, atten_y, mixed_value = self.global_atten(x_glob)
        gaussian_input = (x_glob, atten_x, atten_y, mixed_value)
        x_out = self.gaussiantrans(gaussian_input)
        x_out = x_out.permute(0, 3, 1, 2).contiguous()

        x_out = self.up(x_out)
        x = self.queeze(torch.cat((x_out, h), dim=1)).permute(0, 2, 3, 1).contiguous()
        x = x[:, :H, :W, :].contiguous()
        x = x.view(B, -1, C)
        return x
...
class EAmodule(nn.Module):
    ...
    def forward(self, x):
        ...
        assert int(np.sqrt(x.shape[1])) ** 2 == x.shape[1], "Token count must be square"
...
class Stem(nn.Module):
    def __init__(self):
        ...
        self.position_embedding = None  # <-- Make dynamic
    ...
    def forward(self, x):
        ...
        # Dynamically create positional embedding if needed
        if self.position_embedding is None or self.position_embedding.shape[1] != x.shape[1]:
            self.position_embedding = nn.Parameter(
                torch.zeros(1, x.shape[1], x.shape[2], device=x.device)
            )
...
configs = {
    "win_size": 4,                                                      # we can reduce it to 2: default = 4
    "head": 8,                                                          # we can reduce it to 4: default = 8
    "axis": [28, 16, 8],
    "encoder": [256, 512],
    "bottleneck": 1024,
    "decoder": [1024, 512],
